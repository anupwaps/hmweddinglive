...
class Admin extends CI_Controller {
		        ...
		        if(!empty($rows))
		        {
		            ...
		            foreach ($rows as $row)
		            {
		                ...
		                $nestedData['options'] = $approve_button."<a href='".base_url()."admin/stories/view_story/".$row->happy_story_id."' id='demo-dt-view-btn' class='btn btn-primary btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('view')."' ><i class='fa fa-eye'></i></a>
		               		...
		               		<a href='".base_url()."admin/stories/edit_stories/".$row->happy_story_id."' id='demo-dt-edit-btn' class='btn btn-primary  btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('edit')."' ><i class='fa fa-edit'></i></a>
		               	";
	...
	function general_settings($para1="")
	{
		if ($this->admin_permission() == FALSE) {
			...
			elseif ($para1=="update_general_settings") {
				...
				$data6['value'] = $this->input->post('member_approval_by_admin');
				...
				$data8['value'] = $this->input->post('working_day');
				$this->db->where('type', 'working_day');
				$this->db->update('general_settings', $data8);
				
				$data9['value'] = $this->input->post('working_hours');
				$this->db->where('type', 'working_hours');
				$this->db->update('general_settings', $data9);

				$data10['value'] = $this->input->post('app_store_apple');
				$this->db->where('type', 'app_store_apple');
				$this->db->update('general_settings', $data10);
				
				$data11['value'] = $this->input->post('play_store_google');
				$this->db->where('type', 'play_store_google');
				$this->db->update('general_settings', $data11);

				$data12['value'] = $this->input->post('slider_heading');
				$this->db->where('type', 'slider_heading');
	...
	function gallery_settings($para1="", $para2="")
	{
		if ($this->admin_permission() == FALSE) {
        	redirect(base_url().'admin/login', 'refresh');
		}
			...
			if ($para1=="") {
				...
				$page_data['file'] = "gallery_settings.php";
				$page_data['bottom'] = "theme_color_settings/index.php";
				$page_data['page_name'] = "gallery_settings";
				...
				$page_data['gallery'] = $this->Rest_model->SelectDataOrder('gallery', '*', '','gallery_id','desc');
				$this->load->view('back/index', $page_data);
			}
			elseif ($para1=="add") {
				$data = $this->input->post();
				$config['upload_path'] = './uploads/home_gallery/';
				...
				if (!$this->upload->do_upload('image')) {
					...
					$config_1['source_image'] = 'uploads/home_gallery/'.$data2['upload_data']['file_name'];
					$this->image_lib->initialize($config_1); 
					$this->image_lib->resize();
				}
				$this->Rest_model->SaveData('gallery', $data);
				recache();
				$this->session->set_flashdata('success', "Successfully Added"); 
				redirect(base_url().'admin/gallery_settings', 'refresh');
			}
			elseif ($para1=="delete" && $para2) {
				$qq=$this->Rest_model->SelectData_1('gallery','*',array('gallery_id'=>$para2));
					if($qq->image!=''){
					unlink('uploads/home_gallery/'.$qq->image);}
				$this->Rest_model->DeleteData('gallery', array('gallery_id'=>$para2));
	...
	function test()
	{
		// $test = 'member_id'=>'approved';
		// $this->db->get_where('member',array('member_id'=>1,$test));
	}
}
